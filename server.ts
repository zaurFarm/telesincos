...
export async function requireAuth(req: AuthRequest, res: express.Response, next: express.NextFunction) {
  ...
  const _a = Buffer.from(token || ''); const _b = Buffer.from(expectedToken || ''); if (token && _a.length === _b.length && crypto.timingSafeEqual(_a, _b) && (req.originalUrl || '').startsWith('/api/external/')) {
  ...
  try {
      ...
      if (decoded?.id && decoded.id !== 'admin') {
        const vr = await pool.query('SELECT token_version FROM saas_users WHERE id = $1', [decoded.id]);
        if (vr.rows.length === 0) return res.status(401).json({ error: 'unauthorized' });
        if ((vr.rows[0].token_version || 0) !== (decoded.tv || 0)) return res.status(401).json({ error: 'token_revoked' });
      }
      req.user = decoded;
      next();
  } catch (err) {
...
app.post('/api/auth/login', strictLimiter, async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user.id, email: user.email, plan: user.plan, tenantId: user.workspace_id || user.id, tv: user.token_version || 0 },
      process.env.JWT_SECRET!,
      { expiresIn: '7d' }
    );
...
app.get('/api/auth/2fa/status', requireAuth, async (req: any, res: any) => {
  const r = await pool.query('SELECT totp_enabled FROM saas_users WHERE id = $1', [req.user?.id]);
